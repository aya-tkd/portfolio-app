...
- ユーザーの確定事項、AIの提案、未決事項を区別する。返答がない提案を合意済みにしない。
- Issue開発では専用ブランチを使い、承認済みの範囲は継続して進める。権限と承認条件の正本は[Issue中心の開発フロー](docs/development/issue-workflow.md)とする。
...
新規・変更するクラス、Vueコンポーネント、主要モジュールには、責務・呼び出し元・入出力・重要な設計意図を日本語でコメントします。[コードの読みやすさと解説ルール](docs/development/code-readability.md)に従い、実装後は実ファイルと処理順でWeb初学者向けに説明します。
...
- [Issue中心の開発フロー](docs/development/issue-workflow.md)
- [設計Doc索引](docs/design/INDEX.md)
- [ユーザー技術理解プロファイル](docs/learning/user-technical-profile.md)
- [AIとの協働方針](docs/ai/working-agreement.md)
- [AI Skill一覧](docs/ai/skill-catalog.md)
