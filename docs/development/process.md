...
1. 要件を親Issueに整理する
2. Issue専用ブランチと設計・テスト・セッションログのSub-issueを準備する
3. 設計Sub-issueでHuman Gateを通過する
4. 実装、テスト、設計Docの更新を行う
5. PRを作成し、記録を確定したセッションログをCloseする。人間がコード・設計Doc・モック・テスト結果を確認する
6. 人間が受入・mergeし、親IssueをCloseする
7. AIがmerge・完了を確認し、対象リモート／ローカルブランチを削除して更新済みmainへ戻る

要件・設計・実装・テスト・PR対応の各工程完了前にFBを回収し、承認範囲内のSkills・Docs小改善を同じIssue/PRへ反映・検証する。merge後の通常処理では新しい改善差分を作らない。安全な削除条件と例外はIssue中心の開発フローに従う。
