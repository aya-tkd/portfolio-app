...
設備予約だけの場合は親なしで表示。受付後は予約を消さず受付側の行へ集約する。診察予約を採用する際は、有効な医師ユーザーを選択して`doctor_user_id`と表示名スナップショットを保存する。既存の`doctor_name`だけの予約は履歴として残し、受付時に選び直す。関連削除は制限。設備実施は別テーブルに保存し、実施状態を予約のstatusへ混ぜない。
