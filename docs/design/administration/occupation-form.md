...
職種（医師・看護師など）の基本情報を登録・編集する。受付で担当医候補に含めるかを、管理コードで明示する。この画面では認証・認可、資格確認、職種と権限の対応付けを扱わない。
...
| 受付担当医区分 | はい | `physician`（医師として扱う）/ `other`（対象外）。初期値は`other`。受付候補の判定にだけ用い、権限は表さない |
...
- 職種名未入力、101文字、表示順0、不正な受付担当医区分を保存できない。
- 受付担当医区分を「医師として扱う」にした有効な職種の有効ユーザーだけが、受付の担当医候補になる。
