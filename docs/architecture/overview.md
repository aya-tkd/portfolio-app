...
Vue＋Rails＋SQLiteで患者検索・各マスタ登録に加え、外来一覧と診察・設備の進捗更新を実装しています。予約/受付登録と会計処理は後続Issueです。公開リポジトリですが、アプリ自体はローカル・架空データ限定です。

Issue #38ではOutpatient::ListQueryが予約と受付を選択し、RowPresenterが一覧DTOへ変換します。Outpatient::Advanceが受付versionと状態を検証して診察・設備の更新を確定します。通常の単一DB内で責務を分ける構成であり、別サービスや別読取DBは設けません。
