...
承認済み画面設計・HTMLモック、既存の参照画面、実装の表示結果を照合する独立レビューSkillです。

- 画面の見た目だけでなく、設計記載事項・ACとの対応も証拠付きで確認する
- 同じ画面幅・代表データで比較し、文言、共通部品、配置、折返し、スクロール、操作到達性を確認する
- レビュー専用であり、実装やテストの合否を独断で変更しない。必要入力が欠けた場合は未完了と報告する
